...
drop table emp purge ;	//쓰레기값없이 완전삭제
CREATE TABLE EMP ( 
  EMPNO     NUMBER(4, 0), 
  ENAME     VARCHAR2(10), 
  JOB       VARCHAR2(9), 
...
CREATE UNIQUE INDEX PK_EMP ON EMP (EMPNO) ;	//같은값 있을 시 오류 발생
ALTER TABLE EMP ADD  CONSTRAINT PK_EMP PRIMARY KEY (EMPNO);	//기본키 설정
...
ALTER TABLE EMP ADD CONSTRAINT FK_DEPTNO FOREIGN KEY (DEPTNO) REFERENCES DEPT (DEPTNO);	//외래키 설정
...
(profno number(4) primary key,
 name  varchar2(15) not null,		//한글은 한 자당 3
 id  varchar2(15) not null,		//영어는 한 자당 1
...
insert into hakjum values ('D+',66,69);
insert into hakjum values ('D',60,65);
